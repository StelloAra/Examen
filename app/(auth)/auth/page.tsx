...
function AuthForm() {
  ...
  const [error, setError] = useState<string | null>("Not validated yet");
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (isSignUp) {
        console.log("Submitting signup form with data:", inputValues);

        // CHECK REQUIRED FIELDS (correct logic)
    ...
    <div className="min-h-screen bg-linear-to-br from-cyan-50/30 via-sky-50/20 to-blue-50/30 dark:from-cyan-950/10 dark:via-sky-950/10 dark:to-blue-950/10">
      ...
      <div className="relative w-full max-w-md sm:max-w-lg md:max-w-2xl lg:max-w-4xl mx-auto flex flex-col justify-center min-h-screen p-3 xs:p-4 sm:p-6 lg:p-8">
        <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm rounded-2xl xs:rounded-3xl shadow-2xl border border-cyan-200/50 dark:border-cyan-800/50 p-4 xs:p-6 sm:p-8 lg:p-12">
          ...
          {/* Form */}
          {isLoadning ? (
            <div className="flex flex-col items-center justify-center h-48 xs:h-56 sm:h-64 space-y-3 xs:space-y-4">
              <div className="w-12 h-12 xs:w-14 xs:h-14 sm:w-16 sm:h-16  rounded-xl xs:rounded-2xl bg-linear-to-r from-cyan-500 via-sky-500 to-blue-500 flex items-center justify-center">
                <Loader2 className="h-6 w-6 xs:h-7 xs:w-7 sm:h-8 sm:w-8 animate-spin text-white" />
              </div>
              <p className="text-gray-600 dark:text-gray-300 text-base xs:text-lg sm:text-xl">
                Loading form...
              </p>
            </div>
          ) : (
